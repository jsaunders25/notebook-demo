...
Notebook::Notebook(QString open_file, bool new_file, QWidget *parent)
    : QMainWindow(parent), ui(new Ui::Notebook) {
  ui->setupUi(this);
  ...
  connect(clipboard, &QClipboard::dataChanged, this,
          &Notebook::clipboardChanged);

  // Read Settings and Apply CommandLine Args
  QFont font;
  NotebookSettings::readSettings(&m_current_file_path, &font, &m_load_recent,
  ...
  if (m_current_file_path != UNTITLED && !m_current_file_path.isEmpty() &&
      open_file.isEmpty() && !new_file && m_load_recent)
...
void Notebook::newFile() {
  ...
  if (!ui->textEdit->toPlainText().isEmpty()) {
    ...
    if (save == QMessageBox::Yes) saveAsFile();
  }
...
void Notebook::saveFile() {
  ...
  } else  // If file doesn't exist, prompt user to save as a new file
  {
    if (QMessageBox::Yes ==
        QMessageBox::question(
            this, tr("Save As New File?"),
            tr("Would you like to save this document as a new file?"))) {
      saveAsFile();
    }
  }
}
...
void Notebook::saveAsFile() {
  QFile file(QFileDialog::getSaveFileName(this, tr("Save File"), m_current_dir,
                                          tr("Text (*.txt)")));
  if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
    QTextStream out(&file);
    out << ui->textEdit->toPlainText();
...
void Notebook::on_actionOpen_triggered() { openFileDialog(); }
...
void Notebook::on_actionCopy_triggered() { ui->textEdit->copy(); }

void Notebook::on_actionPaste_triggered() { ui->textEdit->paste(); }

void Notebook::on_actionCut_triggered() { ui->textEdit->cut(); }

void Notebook::on_actionUndo_triggered() { ui->textEdit->undo(); }

void Notebook::on_actionRedo_triggered() { ui->textEdit->redo(); }

void Notebook::on_actionSave_triggered() { saveFile(); }

void Notebook::on_actionSave_As_triggered() { saveAsFile(); }

void Notebook::on_actionPrint_triggered() { print(); }

void Notebook::on_actionNew_triggered() { newFile(); }
...
void Notebook::cursorMoved() {
  ...
  ui->statusbar->showMessage("Ln " + QString::number(cursor.blockNumber() + 1) +
                             ", Col " +
...
void Notebook::on_actionFind_triggered() { find_dialog->openFindDialog(); }
...
void Notebook::on_actionPreferences_triggered() {
  ...
  connect(&dialog, &PreferencesDialog::emitPreferences, this,
          &Notebook::updatePreferences);
  dialog.exec();
}

void Notebook::updatePreferences(bool load_recent, bool word_wrap,
                                 bool status_bar) {
